...
psql -d gis -c "CREATE OR REPLACE VIEW lowzoom_water AS SELECT ST_SimplifyPreserveTopology(way,150) AS way,get_localized_placename(name,\"name:de\",int_name,\"name:en\",true) as name,\"natural\",waterway,way_area FROM planet_osm_polygon WHERE (\"natural\" = 'water' OR waterway = 'riverbank' OR water='lake' OR landuse IN ('basin','reservoir')) AND way_area > 50000;"
...
psql -d gis -c "CREATE OR REPLACE VIEW lowzoom_lakelabel    AS SELECT arealabel(osm_id,way) AS way,get_localized_placename(name,\"name:de\",int_name,\"name:en\",true) as name,'lakeaxis'::text    AS label,way_area FROM planet_osm_polygon WHERE (\"natural\" = 'water' OR water='lake' OR landuse IN ('basin','reservoir')) AND name IS NOT NULL;"
...
psql -d gis -c "CREATE OR REPLACE VIEW lowzoom_glacierlabel AS SELECT arealabel(osm_id,way) AS way,get_localized_placename(name,\"name:de\",int_name,\"name:en\",true) as name,'glacieraxis'::text AS label,way_area FROM planet_osm_polygon WHERE  \"natural\" = 'glacier' AND name IS NOT NULL;"
...
echo "lowzoom_bayllabel..."
psql -d lowzoom -c "INSERT INTO lakelabels SELECT * FROM dblink('dbname=gis','SELECT * FROM lowzoom_bayllabel')     AS t(way geometry(LineString,3857), name text, label text, way_area real);"
...
psql -d gis -c "CREATE OR REPLACE VIEW lowzoom_seaplabel   AS SELECT ST_MakeLine(way, way) as way,get_localized_placename(name,\"name:de\",int_name,\"name:en\",true) as name,'sea'::text as label, 2E10 as way_area FROM planet_osm_point WHERE place='sea' AND name is NOT NULL;"
...
#psql -d gis -c "DROP MATERIALIZED VIEW IF EXISTS lowzoom_natural_areas_mv; CREATE MATERIALIZED VIEW lowzoom_natural_areas_mv AS SELECT natural_arealabel(osm_id,way) AS way,get_localized_placename(name,"name:de",int_name,"name:en",true) AS name,COALESCE("natural","region:type") AS areatype,way_area,OTM_Next_Natural_Area_Size(osm_id,way_area,way) AS hierarchicregions FROM planet_osm_polygon WHERE ("region:type" IN ('natural_area','mountain_area','mountain_range','basin') OR "natural" IN ('massif','mountain_range','basin','valley','couloir','ridge','arete','gorge','gully','canyon')) AND name IS NOT NULL;"
psql -d gis -c "DROP VIEW IF EXISTS lowzoom_natural_areas;"
#psql -d gis -c "DROP MATERIALIZED VIEW IF EXISTS lowzoom_natural_lines_mv; CREATE MATERIALIZED VIEW lowzoom_natural_lines_mv AS SELECT way,get_localized_placename(name,"name:de",int_name,"name:en",true) AS name,"natural" AS areatype,ST_Length(way)*ST_Length(way)/10 AS way_area,OTM_Next_Natural_Area_Size(osm_id,0.0,way) AS hierarchicregions FROM planet_osm_line li WHERE "natural" IN ('massif','mountain_range','basin','valley','couloir','ridge','arete','gorge','gully','canyon') AND name IS NOT NULL AND NOT EXISTS (SELECT 1 FROM planet_osm_polygon po WHERE po.osm_id=li.osm_id);"
...
#psql -d gis -c "INSERT INTO naturalarealabels SELECT (hierarchicregions).way,name,areatype,way_area,(hierarchicregions).nextregionsize,(hierarchicregions).subregionsize FROM lowzoom_natural_lines_mv;"
#psql -d gis -c "INSERT INTO naturalarealabels SELECT (hierarchicregions).way,name,areatype,way_area,(hierarchicregions).nextregionsize,(hierarchicregions).subregionsize FROM lowzoom_natural_areas_mv;"
